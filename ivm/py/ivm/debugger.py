...
from ivm.tree import Tree, Erase, N32Node, F32Node, CombNode, ExtFnNode, BranchNode, VarNode, GlobalNode, Net
...
def find_free_wires(host: Host) -> list[Wire]:
    ...
    return [
        v
        for k, v in parent_status.items()
        if v is not None
    ]
...
class TreeView(RichLog):
    tree: Tree = reactive(Erase(None), recompose=True)
    ...
    def _render_tree(self, n: Tree, width: int, indention: int) -> Iterator[Text]:
        ...
        if n.has_children:
            if len(s) + indention > width:
                yield Text.assemble(" " * indention, (n.head(), tree_colors[type(n)]), "(")
                for c in n:
        ...
        yield (Text.assemble(
            ...
            *(
                ...
                for part in (*(" " for _ in range(i > 0)), *self._render_tree(c, width, 0))
            ),
            *(")" for _ in range(n.has_children))
        ))
    ...
    def on_resize(self, event: Resize) -> None:
        ...
        self.clear()
        for part in self._render_tree(self.tree, width, 0):
            self.write(part)
        super().on_resize(event)

    def watch_tree(self, tree: Tree):
...
class HistoryChart(Widget):
    ...
    history: History | None = reactive(None, recompose=True)
...
def track_history(host: Host) -> list[History]:
    ...
    def add_history():
        history.append(History(
            ...
            stack=tuple((reader.read_port(a, shallow=False), reader.read_port(b, shallow=False)) for stack in (host.ivm.active_slow, host.ivm.active_fast) for a, b in stack),
            stdout=host.stdout.buffer.getvalue().decode('utf-8') if isinstance(host.stdout.buffer, io.BytesIO) else "",
            stderr=host.stderr.buffer.getvalue().decode('utf-8') if isinstance(host.stderr.buffer, io.BytesIO) else "",
            stdin=host.stdin.buffer.getvalue().decode('utf-8') if isinstance(host.stdin.buffer, io.BytesIO) else "",
        ))

    def on_start_interaction(a: Port, b: Port, interaction: str):
        interaction_stack.append(
            (reader.read_port(a, shallow=False), reader.read_port(b, shallow=False), interaction))
        add_history()
