...
class Reader:
    ...
    def read_port(self, p: Port, shallow: bool = True) -> Tree:
        ...
        if isinstance(p, WirePort):
            ...
            p1, p2 = p.aux()
            return CombNode(p.label, self.read_wire(p1), self.read_wire(p2), p.trace)
        elif isinstance(p, ExtFnPort):
            p1, p2 = p.aux()
            return ExtFnNode(p.label, self.read_wire(p1), self.read_wire(p2), p.trace)
            ...
            if isinstance(bp, BranchPort):
                ...
                return CombNode("?^", self.read_wire(p1), self.read_wire(p2), p.trace)
        else:
